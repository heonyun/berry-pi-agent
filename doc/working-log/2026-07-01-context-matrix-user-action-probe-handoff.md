...
date: 2026-07-01
updated: 2026-07-01 (post-#72 fix verification)
author: cursor-agent
...
- **Root cause:** Legacy ottom-composer and 2-status-bar classes applied absolute/fixed positioning so the composer/status strip overlapped the side panel and intercepted pointer events on Save and Recent Range controls.
...
- **Verification:** matrix-grid.spec.ts 9/9, smoke.spec.ts 2/2, probe-followup-isolated.mjs ? detail-save-user-click and ecent-range-user-click **passed** (F2 / row-column header probes still fail; tracked as #73 / #71).
## Work Completed
...
| Implementation PR for #72 | branch `cursor/fix-matrix-layout-click-interception`; fix removes legacy `bottom-composer` / `v2-status-bar` absolute overlay; probe `detail-save-user-click` + `recent-range-user-click` PASS |
